...
abstract class P05CALENDARJOBGETDATA_Event {}

class P05CALENDARJOBGETDATA_GET extends P05CALENDARJOBGETDATA_Event {}

class P05CALENDARJOBGETDATA_GET2 extends P05CALENDARJOBGETDATA_Event {}

class P05CALENDARJOBGETDATA_GET3 extends P05CALENDARJOBGETDATA_Event {}

class P05CALENDARJOBGETDATA_FLUSH extends P05CALENDARJOBGETDATA_Event {}

class P05CALENDARJOBGETDATA_Bloc extends Bloc<P05CALENDARJOBGETDATA_Event, List<P05CALENDARJOBGETDATAclass>> {
  P05CALENDARJOBGETDATA_Bloc() : super([]) {
    on<P05CALENDARJOBGETDATA_GET>((event, emit) {
      return _P05CALENDARJOBGETDATA_GET([], emit);
    });

    on<P05CALENDARJOBGETDATA_GET2>((event, emit) {
      return _P05CALENDARJOBGETDATA_GET2([], emit);
    });
    on<P05CALENDARJOBGETDATA_GET3>((event, emit) {
      return _P05CALENDARJOBGETDATA_GET3([], emit);
    });
    on<P05CALENDARJOBGETDATA_FLUSH>((event, emit) {
      return _P05CALENDARJOBGETDATA_FLUSH([], emit);
    });
  }

  Future<void> _P05CALENDARJOBGETDATA_GET(
      List<P05CALENDARJOBGETDATAclass> toAdd, Emitter<List<P05CALENDARJOBGETDATAclass>> emit) async {
    // FreeLoadingTan(P05CALENDARJOBMAINcontext);
    List<P05CALENDARJOBGETDATAclass> output = [];
    //-------------------------------------------------------------------------------------
    try {
      final response = await Dio().post(
        "$ToServer/02SALTSPRAY/CalendarInMonth",
        data: {
          "month": P05CALENDARJOBVAR.dateTimeSelect.month.toString(),
          "year": P05CALENDARJOBVAR.dateTimeSelect.year.toString(),
        },
        options: Options(
      ...
      if (response.statusCode == 200) {
        ...
        // Navigator.pop(P01DASHBOARDMAINcontext);
        showErrorPopup(P05CALENDARJOBMAINcontext, response.toString());
        output = [];
        emit(output);
      }
  ...
  Future<void> _P05CALENDARJOBGETDATA_GET2(
      List<P05CALENDARJOBGETDATAclass> toAdd, Emitter<List<P05CALENDARJOBGETDATAclass>> emit) async {
    // List<P05CALENDARJOBGETDATAclass> output = [];
    //-------------------------------------------------------------------------------------
    // var input = dummydatainput2;

    // List<P05CALENDARJOBGETDATAclass> outputdata = input
    //     .where((data) =>
    //         data['location'] == 'ESIE1' &&
    //         data['plant'] == 'YES' &&
    //         data['step01'] == 'YES')
  ...
  Future<void> _P05CALENDARJOBGETDATA_GET3(
      List<P05CALENDARJOBGETDATAclass> toAdd, Emitter<List<P05CALENDARJOBGETDATAclass>> emit) async {
    // List<P05CALENDARJOBGETDATAclass> output = [];
    //-------------------------------------------------------------------------------------
    // List<P05CALENDARJOBGETDATAclass> datadummy = [
    //   P05CALENDARJOBGETDATAclass(
    //     PLANT: "PH PO:1234",
    //     STEP01: "YES",
    //     STEP02: "YES",
    //     STEP03: "YES",
  ...
  Future<void> _P05CALENDARJOBGETDATA_FLUSH(
      List<P05CALENDARJOBGETDATAclass> toAdd, Emitter<List<P05CALENDARJOBGETDATAclass>> emit) async {
    List<P05CALENDARJOBGETDATAclass> output = [];
    emit(output);
  }
...
class P05CALENDARJOBGETDATAclass {
  P05CALENDARJOBGETDATAclass({
    this.REQUESTNO = '',
    this.REPORTNO = '',
    this.SECTION = '',
